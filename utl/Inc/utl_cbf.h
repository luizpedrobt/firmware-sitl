...
#define UTL_CBF_DECLARE(name, _size)                                                                                   \
    ...
    /**
     @brief Retorna a quantidade de bytes disponível para consumo num buffer
     circular.
    ...
    */
    ...
    /**
     ...
     A macro @ref CBF_DECLARE já faz esse papel mas essa função pode ser usada para
     inicialização de forma independente da macro.
     @param[in] cb - ponteiro para o buffer circular.
     @param[in] area - buffer previamente alocado que será usado para armazenamento
     do conteúdo do buffer circular.

...
typedef void (*hal_rtc_alarm_cbk_t)(uint32_t id);

typedef struct hal_rtc_driver_s
{
	void (*nvram_write)(uint32_t index, uint32_t value);
	uint32_t (*nvram_read)(uint32_t index);
	void (*set)(hal_rtc_date_time_t *dt);
	void (*get)(hal_rtc_date_time_t *dt);
	void (*alarm_set)(hal_rtc_alarm_cbk_t alarm_cbk, uint32_t alarm_id, hal_rtc_alarm_type_t alarm_type, hal_rtc_weekday_t week_day, uint8_t hour_24, uint8_t minute, uint8_t second);
	void (*alarm_reset)(void);
	void (*shutdown)(hal_rtc_alarm_type_t alarm_type, hal_rtc_weekday_t week_day, uint8_t hour_24, uint8_t minute, uint8_t second);
	void (*init)(void);
} hal_rtc_driver_t;
...
void hal_rtc_alarm_set(hal_rtc_alarm_cbk_t alarm_cbk, uint32_t alarm_id, hal_rtc_alarm_type_t alarm_type, hal_rtc_weekday_t week_day, uint8_t hour_24, uint8_t minute, uint8_t second);
void hal_rtc_shutdown(hal_rtc_alarm_type_t alarm_type, hal_rtc_weekday_t week_day, uint8_t hour_24, uint8_t minute, uint8_t second);
